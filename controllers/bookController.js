...
exports.getAllBooks = async (req,res) => {
 ...
 exports.getBook = async (req, res) => {
    try {
      const user = await User.findById(req.session.userID); /*Kullanıcıyı yakaladım. Eğer kullanıcı kitabı ödünç almışsa tekrar
       almaması için if koşulu yapmam gerekecek bunun için user bilgileri gerekiyordu. */
      const book = await Book.findOne({slug: req.params.slug}).populate('user');/* Populate işlemi yaptım burada kullanıcı bilgilerinide
